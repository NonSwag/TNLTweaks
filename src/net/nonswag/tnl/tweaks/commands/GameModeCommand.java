        ...
        if (sender instanceof Player) {
            if (args.length >= 1) {
                ...
                if (args.length >= 2) {
                    ...
                    if (player == null) {
                        sender.sendMessage(ChatComponent.getText("%prefix%§4 " + args[1] + "§c is unknown to us"));
                        return false;
                    } else if (!player.isOnline()) {
                        sender.sendMessage(ChatComponent.getText("%prefix%§4 " + player.getName() + "§c is Offline"));
                        return false;
                    }
                } else {
                ...
                if (player.getGameMode().equals(gameMode)) {
                    ...
                    player.sendMessage(ChatComponent.getText("%prefix%§a Your gamemode is now §6" + player.getGameMode().name().toLowerCase()));
                    if (!player.getName().equalsIgnoreCase(sender.getName())) {
                        sender.sendMessage(ChatComponent.getText("%prefix%§6" + player.getName() + "'s§a gamemode is now §6" + player.getGameMode().name().toLowerCase()));
                    }
                }
                return true;
            } else {
